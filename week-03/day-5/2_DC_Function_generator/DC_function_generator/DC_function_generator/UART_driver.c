...
void UART_SendCharacter(char character) {
    ...
    // See page 247 of the datasheet for hints, be aware that the code in the 
	// datasheet has a problem :)
...
char UART_GetCharacter() {
    // Wait for data in the circular buffer, this can be detected if the write and 
	// read pointers are pointing to the same memory block
