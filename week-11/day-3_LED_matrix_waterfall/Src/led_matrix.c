...
void led_matrix_update_thread(void const *argument)
{
	...
	GPIO_InitDef.Pin = GPIO_PIN_15 | GPIO_PIN_14 | GPIO_PIN_9 | GPIO_PIN_8 | GPIO_PIN_4 | GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_1 | GPIO_PIN_0 | GPIO_PIN_3;// add all used pins! 0, 1, 3, 4, 6, 7, 8, 9, 14, 15
...
//	led_matrix_set(1,1,1);
//	led_matrix_set(2,2,1);

	while (1) {
		// TODO:
		// Implement the led matrix updater functionality
		// Step 1:
		// Iterate through every column or row / COL, 'cause col is pulled up with resistor!
		for (int ci = 0; ci < LED_MATRIX_COLS; ci++) {
			...
			HAL_GPIO_WritePin(ledmatrixcoloumns[ci].GPIOx, ledmatrixcoloumns[ci].PIN, GPIO_PIN_SET); // az i-edik oszlopot állítsuk SET-re

			// Step 4:
			// Turn on the leds in that column or row / ROW! - ehhez a sort le kell nullázni
			for (int ri = 0; ri < LED_MATRIX_ROWS; ri++) {
				HAL_GPIO_WritePin(ledmatrixrows[ri].GPIOx, ledmatrixrows[ri].PIN, !led_matrix_state[ri][ci]); // ez a waterfallhoz kellene
				//HAL_GPIO_WritePin(ledmatrixrows[ri].GPIOx, ledmatrixrows[ri].PIN, GPIO_PIN_RESET); //ez NEM JÓ!
			}
			// Step 5:
			// Release the mutex
			osMutexRelease(led_matrix_mutex_id);
			...
			// Step 7:
			// Turn off the column or row / COL
			HAL_GPIO_WritePin(ledmatrixcoloumns[ci].GPIOx, ledmatrixcoloumns[ci].PIN, GPIO_PIN_RESET);
		}
/*
		// Step 1:
		...
		for (int ri = 0; ri < LED_MATRIX_ROWS; ri++) {
			...
			// Step 3:
			// Turn on the row
			HAL_GPIO_WritePin(ledmatrixrows[ri].GPIOx, ledmatrixrows[ri].PIN, GPIO_PIN_RESET);

			// Step 4:
			// Turn on the leds in that column or row / ROW! - ehhez a sort le kell nullázni
			for (int ci = 0; ci < LED_MATRIX_COLS; ci++) {
				HAL_GPIO_WritePin(ledmatrixcoloumns[ci].GPIOx, ledmatrixcoloumns[ci].PIN, led_matrix_state[ci][ri]);
				//HAL_GPIO_WritePin(ledmatrixcoloumns[ci].GPIOx, ledmatrixcoloumns[ci].PIN, GPIO_PIN_SET); //EZ NEM JÓ ÍGY!
			}
			// Step 5:
			// Release the mutex
			osMutexRelease(led_matrix_mutex_id);
			...
			// Step 7:
			// Turn off the column or row / COL
			HAL_GPIO_WritePin(ledmatrixrows[ri].GPIOx, ledmatrixrows[ri].PIN, GPIO_PIN_SET);
		}
*/
	}//while
