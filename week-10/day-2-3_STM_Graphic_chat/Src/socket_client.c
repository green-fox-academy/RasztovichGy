...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define SERVER_IP         	"127.0.0.1" 	// cmd \ ipconfig \ WiFi IPv4 address 10.27.6.199
...
int sentdatasize;					// Size of sent message.
	...
	// Establish connection to the server
		...
		while (1) {
			do {
				...
				sentdatasize = send(clientsocket, &msgtosend, sizeof(msgtosend),0);
				// buffer[sentdatasize] = '\0';
				osDelay(200);
